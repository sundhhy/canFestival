...
#define AM335X_CAN_NUM_MAILBOX			64
#define AM335X_CAN_RX_NUM	(48)
...
/* spruh73c.pdf: 23.3.9.1.4 Oscillator Tolerance Range:
 * It has to be considered that SJW may not be larger than the smaller of the phase buffer
 * segments and that the propagation time segment limits that part of the bit time that may
 * be used for the phase buffer segments.
*/
...
#define AM335X_CAN_BITRATE_BRP_125K_DEFAULT		 16
#define AM335X_CAN_BITRATE_BRP_250K_DEFAULT		  8
...
#define AM335X_DCAN_ES_EOI			0x04	// Error and Status Register
	...
	#define	AM335X_DCAN_ERR_BBREAK			(1<<21)		// Data beyond the buffer
	#define	AM335X_DCAN_DATA_OVERFLOW		(1<<22)		// Data beyond the buffer
	#define	AM335X_DCAN_DATA_REV			(1<<23)		// Data beyond the buffer
...
#define INIT_FLAGS_DISAUTORTRNSMSSN		0x00001000  /* Automatic retransmission disabled */
...
#define MAXMSGNUM	512	//sundh:add
#define SendTimeOut	100
#define MAXRESTARTTIME	200
#define DELAYTIME	118	//sundh:delay 60 sec
#define NODE_ACTIVE		8

#define NODE_UNACTIVE	0
#define OUT_TIME		10	//加大超时时间量，缓解主从切换的问题
#define FLOOR_LIMIT		1
...
typedef struct candev_am335x_entry
{
    ...
    int 						txmbfullcount;	/* sundh 140429	连续的发送邮箱满了的时候计数	*/
    volatile uint32_t           dflags;     /* Device specific flags */
    CANDEV_AM335X_INFO         *devinfo;   /* Common device information */
} CANDEV_AM335X;


/* Device Activity Parameter of CANDEV struct */
typedef struct candev_am335x_Activity_Parameter
{

	uint8_t				MsgHead[2];
	uint8_t				MsgTail[2];
	appMessage_t			DCAN_REV_DATA[2][MAXMSGNUM];
	uint32_t		Errst_Reg[2];
	uint32_t		INT_Reg[2];
	uint32_t		EOI_LEC_BIT0_COUNT[2];
	uint32_t		EOI_LEC_BIT1_COUNT[2];
	uint32_t		EOI_LEC_CRC_COUNT[2];
	uint32_t		ERR_CAN_REVADDR[2];

	uint8_t			 	CAN_UINT;
	uint8_t				CAN_Intr_Attach_Num;
	uint8_t				CAN_SEND_STATUS[2];
	uint8_t				CAN_DEV_Restar_Count[2];
	uint8_t				Restar_Delay[2];
	uint8_t				Node_Active_Flag[2];

	uint8_t				Node_Active_Count[2];

#ifdef PRI_MBXIDCOUNT
	uint32_t			rcvdatmbxid[2][AM335X_CAN_RX_NUM];	//用于记录收到数据的邮箱号
	uint32_t			msglst[2];				//统计丢包量
	uint32_t			rcvintrcount[2];				//接收中断次数
#endif
	/*
	int			 	CAN_UINT;
	int				CAN_Intr_Attach_Num;
	int				CAN_SEND_STATUS[2];
	int				CAN_DEV_Restar_Count[2];
	int				Restar_Delay[2];
	int				Node_Active_Flag[2];
	int				Node_Active_Count[2];
*/

} CAN_DUPLEX_Activity_AM335X;
